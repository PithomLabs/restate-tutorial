...
func (UserSession) Checkout(ctx restate.ObjectContext, orderID string) (bool, error) {
	...
	_, err = restate.Run(ctx, func(ctx restate.RunContext) (bool, error) {
		...
		//    "awakeable_id": id,
		//    "amount_cents": 10000,
		//    "callback_url": "https://ingress.example.com/api/v1/awakeable/{id}/resolve"
		// }
		return true, nil
	})
	if err != nil {
...
// This helper will POST to ingress's awakeable resolution endpoint.
func ResolveAwakeableManually(restateURL, id string, receipt models.PaymentReceipt) error {
	// Compose URL to ingress awakeable resolve endpoint
	url := fmt.Sprintf("%s/api/v1/awakeable/%s/resolve", restateURL, id)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return fmt.Errorf("ingress returned non-2xx status: %d", resp.StatusCode)
	}
	return nil
}
